...
from downtrail import send_msg
...
weight = ROOT / 'weights/best_whole.pt'
is_interrupt = []
         ...
         if is_interrupt[t_id] == False:
            ...
            for i, det in enumerate(pred):
                ...
                if len(det):
                    ...
                    picture_name = 'act_' + str(t_id) + '_' + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + 'A001' + '.jpg'
                    ...
                    if frame % fnum == 0 and a0 != 0 and max_conf >= 0.5:
                        ...
                        #    if is_sql[i] >= 0.5 and datetime.time(7, 30) <= datetime.datetime.now().time() <= datetime.time(17, 30) and i  != 2 and i != 3 and i != 5:
                        #        con2sql_hnu(model.names[i], is_sql[i] * 100, vidcut_picture, rtsp_addr, t_id)
                        #    if is_sql[i] >= 0.5 and is_sql[i] < 0.8 and i  != 0 and i != 1:
                        #        con2sql_hnu(model.names[i], is_sql[i] * 100, vidcut_picture, rtsp_addr, t_id)
